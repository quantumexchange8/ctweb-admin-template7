...
(function ($) {
    ...
    function initDropdownMenu() {
        if (document.getElementById("topnav-menu-content")) {
            ...
            for (var i = 0, len = elements.length; i < len; i++) {
                elements[i].onclick = function (elem) {
                    // Get the href without hash (#) or query parameters
                    var linkPath = new URL(elem.target.href, window.location.origin).pathname;
                    var currentPath = window.location.pathname;
    
                    // Check if the paths match and toggle if true
                    if (linkPath === currentPath) {
                        elem.preventDefault(); // Prevent navigation if needed
                        elem.target.parentElement.classList.toggle("active");
